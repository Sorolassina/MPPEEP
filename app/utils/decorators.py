    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(request: Request, *args, **kwargs):
            ...
            if client_id in calls:
                calls[client_id] = [
                    call_time for call_time in calls[client_id] if current_time - call_time < window_seconds
                ]
            else:
                calls[client_id] = []
            ...
            if len(calls[client_id]) >= max_calls:
                raise HTTPException(
                    status_code=429, detail=f"Trop de requêtes. Réessayez dans {window_seconds} secondes."
                )
