...
class BesoinAgent(SQLModel, table=True):
    ...
    periode: Optional[str] = Field(max_length=50)  # Ex: "Trimestre 1", "Semestre 1"
    ...
    grade_id: Optional[int] = Field(default=None, foreign_key="grade_complet.id")  # Grade souhaité
    categorie_souhaitee: Optional[str] = Field(max_length=1)  # A, B, C, D
    ...
    # Justification
    motif: Optional[str] = None  # Raison du besoin
    urgence: Optional[str] = Field(max_length=20, default="Normale")  # Faible, Normale, Élevée, Critique
    
    # Statut
    statut: str = Field(max_length=50, default="Exprimé")  # Exprimé, Transmis, En attente, Partiellement satisfait, Satisfait, Rejeté
    
    # Dates
    date_expression: date = Field(default_factory=date.today)  # Date où le besoin est exprimé
    date_transmission: Optional[date] = None  # Date de transmission au ministère
    date_satisfaction: Optional[date] = None  # Date où le besoin est satisfait
    
    # Metadata
    exprime_par_user_id: Optional[int] = Field(default=None, foreign_key="user.id")  # Qui a exprimé le besoin
    valide_par_user_id: Optional[int] = Field(default=None, foreign_key="user.id")  # Qui a validé
    
    observations: Optional[str] = None  # Commentaires/notes
...
class SuiviBesoin(SQLModel, table=True):
    ...
    # Changement
    ancien_statut: Optional[str] = Field(max_length=50)
    ...
    modifie_par_user_id: Optional[int] = Field(default=None, foreign_key="user.id")
...
class ConsolidationBesoin(SQLModel, table=True):
    ...
    periode: Optional[str] = Field(max_length=50)
    ...
    taux_satisfaction: Optional[Decimal] = Field(default=None, max_digits=5, decimal_places=2)  # Pourcentage
