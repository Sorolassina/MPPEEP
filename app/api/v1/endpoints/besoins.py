...
def besoins_home(
    ...
    total_besoins = session.exec(
        select(func.count(BesoinAgent.id)).where(BesoinAgent.annee == annee_actuelle).where(BesoinAgent.actif)
    ).one()
...
def api_list_besoins(
    ...
    query = select(BesoinAgent).where(BesoinAgent.actif)
    ...
    return [
        {
            ...
            "direction_libelle": directions[b.direction_id].libelle
            if b.direction_id and b.direction_id in directions
            else None,
...
def api_create_besoin(
    ...
    categorie_souhaitee: str | None = Form(None),
...
def api_update_besoin(
    besoin_id: int,
    nombre_obtenu: int | None = Form(None),
    statut: str | None = Form(None),
    date_transmission: str | None = Form(None),
...
def api_delete_besoin(
    besoin_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)
):
...
def api_generer_consolidation(
    ...
    """
    # Requête de base
    query = select(BesoinAgent).where(BesoinAgent.annee == annee).where(BesoinAgent.actif)
...
def api_export_consolidation(consolidation_id: int, session: Session = Depends(get_session)):
...
def api_statistiques_besoins(annee: int | None = None, session: Session = Depends(get_session)):
    """Statistiques globales sur les besoins"""
    if not annee:
        annee = datetime.now().year

    # Stats globales
    besoins = session.exec(select(BesoinAgent).where(BesoinAgent.annee == annee).where(BesoinAgent.actif)).all()
