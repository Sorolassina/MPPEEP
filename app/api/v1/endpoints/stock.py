...
def aide_stock(request: Request):
    ...
    return templates.TemplateResponse("pages/aide_stock.html", get_template_context(request))
...
def stock_articles(
    ...
    articles = session.exec(select(Article).order_by(Article.designation)).all()

    # Récupérer les catégories
    categories = session.exec(select(CategorieArticle).order_by(CategorieArticle.libelle)).all()

    return templates.TemplateResponse(
        "pages/stock_articles.html", get_template_context(request, articles=articles, categories=categories)
    )
...
def stock_mouvements(
    ...
    mouvements = session.exec(select(MouvementStock).order_by(MouvementStock.created_at.desc()).limit(50)).all()

    # Récupérer tous les articles pour le mapping
    articles = session.exec(select(Article)).all()

    return templates.TemplateResponse(
        "pages/stock_mouvements.html", get_template_context(request, mouvements=mouvements, articles=articles)
    )
...
def stock_demandes(
    ...
    demandes = session.exec(select(DemandeStock).order_by(DemandeStock.created_at.desc())).all()

    return templates.TemplateResponse("pages/stock_demandes.html", get_template_context(request, demandes=demandes))
...
def stock_article_new(
    ...
    return templates.TemplateResponse(
        "pages/stock_article_form.html", get_template_context(request, categories=categories)
    )
...
def stock_article_edit(
    ...
    return templates.TemplateResponse(
        "pages/stock_article_form.html", get_template_context(request, article=article, categories=categories)
    )
...
def stock_mouvement_new(
    ...
    fournisseurs = session.exec(select(Fournisseur).where(Fournisseur.actif).order_by(Fournisseur.nom)).all()

    return templates.TemplateResponse(
        "pages/stock_mouvement_form.html", get_template_context(request, articles=articles, fournisseurs=fournisseurs)
    )
...
def stock_fournisseurs(
    ...
    fournisseurs = session.exec(select(Fournisseur).order_by(Fournisseur.nom)).all()

    return templates.TemplateResponse(
        "pages/stock_fournisseurs.html", get_template_context(request, fournisseurs=fournisseurs)
    )
...
def stock_inventaires(
    ...
    inventaires = session.exec(select(Inventaire).order_by(Inventaire.created_at.desc())).all()

    return templates.TemplateResponse(
        "pages/stock_inventaires.html", get_template_context(request, inventaires=inventaires)
    )
...
def stock_rapports(
    ...
    return templates.TemplateResponse("pages/stock_rapports.html", get_template_context(request))
...
def stock_inventaire_new(
    ...
    return templates.TemplateResponse("pages/stock_inventaire_form.html", get_template_context(request))
...
def stock_inventaire_detail(
    ...
    articles_disponibles = session.exec(
        select(Article)
        .where(Article.actif, Article.id.notin_(articles_deja_ajoutes) if articles_deja_ajoutes else True)
...
def api_kpis(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
...
def api_alertes(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
...
def api_list_articles(
    session: Session = Depends(get_session), current_user: User = Depends(get_current_user), rupture: bool | None = None
):
    """Liste des articles"""
    try:
...
def api_create_article(
    ...
    quantite_max: float | None = Form(None),
...
def api_update_article(
    ...
    quantite_min: float | None = Form(None),
    quantite_max: float | None = Form(None),
    prix_unitaire: float | None = Form(None),
    emplacement: str | None = Form(None),
    description: str | None = Form(None),
    ...
    try:
        ...
        return {"success": True, "message": f"Article '{article.designation}' mis à jour avec succès"}
    except Exception as e:
...
def api_delete_article(
    article_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        mouvements_count = session.exec(
            select(func.count(MouvementStock.id)).where(MouvementStock.article_id == article_id)
        ).one()
...
async def api_create_mouvement(
    ...
    quantite: float = Form(...),
    motif: str = Form(...),
    fournisseur_id: int | None = Form(None),
    beneficiaire: str | None = Form(None),
    prix_unitaire_reel: float | None = Form(None),
    ...
    try:
        ...
        if type_mouvement == "SORTIE":
            disponible, msg_alerte = StockService.verifier_stock_disponible(
                session=session, article_id=article_id, quantite_demandee=Decimal(str(quantite))
            )
            if not disponible:
...
async def api_get_mouvement(
    mouvement_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)
):
...
async def api_create_demande(
    ...
    motif: str = Form(...),
    service_demandeur: str | None = Form(None),
    description: str | None = Form(None),
    document: UploadFile | None = File(None),
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
    ...
    try:
        ...
        return {
            ...
            "data": {"id": demande.id, "numero": demande.numero, "document_path": document_path},
        }
    except Exception as e:
...
def api_stats_mouvements_mois(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        mouvements_entree = session.exec(
            ...
            .where(and_(MouvementStock.type_mouvement == "ENTREE", MouvementStock.date_mouvement >= date_debut.date()))
            .group_by("mois", "annee")
            .order_by("annee", "mois")
        ).all()
        ...
        mouvements_sortie = session.exec(
            ...
            .where(and_(MouvementStock.type_mouvement == "SORTIE", MouvementStock.date_mouvement >= date_debut.date()))
            .group_by("mois", "annee")
            .order_by("annee", "mois")
        ).all()
...
def api_stats_articles_categorie(
    ...
    try:
        stats = session.exec(
            select(CategorieArticle.libelle, func.count(Article.id).label("count"))
            .join(Article, Article.categorie_id == CategorieArticle.id)
            .where(Article.actif)
            .group_by(CategorieArticle.libelle)
        ).all()

        return {"success": True, "data": [{"categorie": s.libelle, "count": s.count} for s in stats]}
    except Exception as e:
...
def api_list_categories(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return {
            "success": True,
            "data": [
                {"id": c.id, "code": c.code, "libelle": c.libelle, "description": c.description} for c in categories
            ],
        }
    except Exception as e:
...
def api_generer_code_article(
    categorie_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        count = session.exec(select(func.count(Article.id)).where(Article.categorie_id == categorie_id)).one()

        # Générer le code : CODE_CATEGORIE-XXX
        numero = str(count + 1).zfill(3)
        code_genere = f"{categorie.code}-{numero}"

        return {"success": True, "data": {"code": code_genere, "categorie_code": categorie.code, "numero": count + 1}}
    except Exception as e:
...
def api_create_fournisseur(
    ...
    try:
        ...
        existing = session.exec(select(Fournisseur).where(Fournisseur.code == code)).first()
        ...
        return {"success": True, "message": f"Fournisseur '{nom}' créé avec succès", "data": {"id": fournisseur.id}}
    except Exception as e:
...
def api_get_fournisseur(
    fournisseur_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)
):
...
def api_update_fournisseur(
    ...
    try:
        ...
        return {"success": True, "message": f"Fournisseur '{fournisseur.nom}' mis à jour avec succès"}
    except Exception as e:
...
def api_delete_fournisseur(
    ...
    try:
        ...
        # Vérifier s'il y a des mouvements liés
        mouvements_count = session.exec(
            select(func.count(MouvementStock.id)).where(MouvementStock.fournisseur_id == fournisseur_id)
        ).one()
        ...
        if mouvements_count > 0:
            ...
            return {"success": True, "message": f"Fournisseur '{nom}' supprimé avec succès"}
    except Exception as e:
...
def api_rapport_valorisation(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    """Rapport de valorisation du stock"""
    try:
        # Valorisation totale
        valeur_totale = (
            session.exec(select(func.sum(Article.quantite_stock * Article.prix_unitaire)).where(Article.actif)).one()
            or 0
        )

        nb_articles = session.exec(select(func.count(Article.id)).where(Article.actif)).one()
...
def api_rapport_mouvements(
    mois: int = 6, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        entrees = session.exec(
            select(func.count(MouvementStock.id)).where(
                MouvementStock.type_mouvement == "ENTREE", MouvementStock.date_mouvement >= date_debut
            )
        ).one()

        sorties = session.exec(
            select(func.count(MouvementStock.id)).where(
                MouvementStock.type_mouvement == "SORTIE", MouvementStock.date_mouvement >= date_debut
            )
        ).one()
        ...
        return {
            ...
            "data": {
                ...
                "par_mois": [
                    {"mois": mois_key, "entrees": data["entrees"], "sorties": data["sorties"]}
...
def api_rapport_fournisseurs(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
...
def api_create_inventaire(
    ...
    try:
        ...
        existing = session.exec(select(Inventaire).where(Inventaire.numero == numero)).first()
        ...
        return {"success": True, "message": f"Inventaire '{numero}' créé avec succès", "data": {"id": inventaire.id}}
    except Exception as e:
...
async def api_add_lignes_inventaire(
    ...
    try:
        ...
        for article_id in articles_ids:
            ...
            existing = session.exec(
                select(LigneInventaire).where(
                    LigneInventaire.inventaire_id == inventaire_id, LigneInventaire.article_id == article_id
                )
            ).first()
        ...
        return {"success": True, "message": f"{lignes_creees} article(s) ajouté(s) à l'inventaire"}
    except Exception as e:
...
@router.post(
    "/api/inventaires/lignes/{ligne_id}/compter", response_class=JSONResponse, name="api_compter_ligne_inventaire"
)
def api_compter_ligne_inventaire(
    ...
    try:
        ...
        return {"success": True, "message": "Comptage enregistré avec succès"}
    except Exception as e:
...
def api_delete_ligne_inventaire(
    ...
    try:
        ...
        return {"success": True, "message": "Article retiré de l'inventaire"}
    except Exception as e:
...
def api_cloturer_inventaire(
    inventaire_id: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)
):
    ...
    try:
        ...
        lignes_non_comptees = session.exec(
            select(func.count(LigneInventaire.id)).where(
                LigneInventaire.inventaire_id == inventaire_id, LigneInventaire.quantite_physique.is_(None)
            )
        ).one()

        if lignes_non_comptees > 0:
            return {"success": False, "error": f"Il reste {lignes_non_comptees} article(s) non compté(s)"}
        ...
        return {"success": True, "message": f"Inventaire '{inventaire.numero}' clôturé avec succès"}
    except Exception as e:
...
def stock_lots_perissables(
    ...
    return templates.TemplateResponse(
        "pages/stock_lots_perissables.html", get_template_context(request, current_user=current_user)
    )


@router.get("/api/lots-perissables", response_class=JSONResponse, name="api_list_lots_perissables")
def api_list_lots_perissables(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    """Liste tous les lots périssables"""
    try:
        lots = session.exec(select(LotPerissable).order_by(LotPerissable.date_peremption)).all()
...
def api_create_lot_perissable(
    ...
    quantite: float = Form(...),
    date_fabrication: str | None = Form(None),
    fournisseur_id: int | None = Form(None),
    observations: str | None = Form(None),
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user),
...
def api_alertes_peremption(session: Session = Depends(get_session), current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        lots_alerte = StockService.get_lots_proches_peremption(session=session, jours=60)

        lots_perimes = StockService.get_lots_perimes(session=session)

        data = {"lots_alerte": [], "lots_perimes": []}
...
def stock_amortissements(
    ...
    return templates.TemplateResponse(
        "pages/stock_amortissements.html", get_template_context(request, current_user=current_user)
    )
...
def api_calculer_amortissement(
    ...
    try:
        amortissement = StockService.calculer_amortissement_annee(session=session, article_id=article_id, annee=annee)
...
@router.get(
    "/api/amortissements/materiels-a-amortir/{annee}", response_class=JSONResponse, name="api_materiels_a_amortir"
)
def api_materiels_a_amortir(
    annee: int, session: Session = Depends(get_session), current_user: User = Depends(get_current_user)
):
