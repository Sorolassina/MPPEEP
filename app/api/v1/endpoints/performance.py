...
def get_objectifs_api(
    ...
    type_objectif: Optional[str] = Query(None)
):
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "progression_pourcentage": float(objectif.progression_pourcentage) if objectif.progression_pourcentage else 0,
...
def create_objectif_api(
    ...
    service_responsable: Optional[str] = Form(None),
    indicateurs_associes: Optional[str] = Form(None),
    commentaires: Optional[str] = Form(None),
    notes_internes: Optional[str] = Form(None),
    db: Session = Depends(get_session),
...
def update_objectif_api(
    ...
    description: Optional[str] = Form(None),
    type_objectif: Optional[str] = Form(None),
    ...
    valeur_cible: Optional[str] = Form(None),
    valeur_actuelle: Optional[str] = Form(None),
...
def get_indicateurs_api(
    ...
    responsable_id: Optional[int] = Query(None),
    frequence_mesure: Optional[str] = Query(None)
):
    ...
    try:
        ...
        return {
            "success": True,
            "data": indicateurs_dict,
            "count": len(indicateurs_dict)
        }
...
def create_indicateur_api(
    ...
    seuil_alerte_min: Optional[str] = Form(None),
    seuil_alerte_max: Optional[str] = Form(None),
    responsable_id: int = Form(...),
    service_responsable: Optional[str] = Form(None),
    source_donnees: Optional[str] = Form(None),
    commentaires: Optional[str] = Form(None),
    db: Session = Depends(get_session),
    ...
    try:
        ...
        indicateur = PerformanceService.creer_indicateur(
            session=db,
            indicateur_data=indicateur_data,
...
def update_indicateur_api(
    ...
    frequence_mesure: Optional[str] = Form(None),
    valeur_cible: Optional[str] = Form(None),
    valeur_actuelle: Optional[str] = Form(None),
    unite_mesure: Optional[str] = Form(None),
    seuil_alerte_min: Optional[str] = Form(None),
    seuil_alerte_max: Optional[str] = Form(None),
    responsable_id: Optional[int] = Form(None),
    service_responsable: Optional[str] = Form(None),
...
def generate_report_api(
    ...
    try:
        ...
        user_name = current_user.full_name if hasattr(current_user, 'full_name') and current_user.full_name else \
                    current_user.email if hasattr(current_user, 'email') else "Utilisateur"
        ...
        if format == "PDF":
            ...
            rapport = RapportPerformance(
                ...
                description=f"Rapport de performance généré automatiquement",
            ...
            return {
                "success": False,
                "error": f"Format {format} non encore implémenté. Utilisez PDF pour l'instant."
            }
...
def get_rapports_historique(
    ...
    try:
        ...
        rapports = db.exec(
            select(RapportPerformance)
            .order_by(RapportPerformance.created_at.desc())
            .limit(50)
        ).all()
