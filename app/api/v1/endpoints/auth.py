...
async def login_get(request: Request, error: str = None):
    ...
    return templates.TemplateResponse(
        "auth/login.html", 
        get_template_context(request, error=error)
    )
...
async def login_post(
    ...
    if not user:
        ...
        return templates.TemplateResponse(
            "auth/login.html",
            get_template_context(request, error="Email ou mot de passe incorrect")
        )
    ...
    if not user.is_active:
        ...
        return templates.TemplateResponse(
            "auth/login.html",
            get_template_context(request, error="Compte désactivé. Contactez l'administrateur.")
        )
...
async def verify_code_get(
    ...
    return templates.TemplateResponse(
        ...
        get_template_context(request, email=email, success=success)
    )
...
async def verify_code_post(
    ...
    if code != recovery_data['code']:
        ...
        return templates.TemplateResponse(
            "auth/recovery/verify_code.html",
            get_template_context(request, email=email, error=f"Code incorrect. {5 - recovery_data['attempts']} tentatives restantes.")
        )
...
async def reset_password_get(
    ...
    return templates.TemplateResponse(
        ...
        get_template_context(request, email=email, code=code)
    )
...
async def reset_password_post(
    ...
    if not success:
        return templates.TemplateResponse(
            "auth/recovery/reset.html",
            get_template_context(request, email=email, code=code, error="Erreur lors de la mise à jour du mot de passe.")
        )
