...
def create_user(payload: UserCreate, session=Depends(get_session), current_user=Depends(require_roles("admin"))):
...
def list_users(session: Session = Depends(get_session), current_user=Depends(require_roles("admin"))):
...
def list_users_by_service(
    service_id: int, session: Session = Depends(get_session), current_user=Depends(require_roles("admin", "user"))
):
