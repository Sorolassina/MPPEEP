...
def workflow_config_home(
    request: Request, session: Session = Depends(get_session), current_user=Depends(get_current_user)
):
    """
    ...
    templates_list = session.exec(select(WorkflowTemplate).order_by(WorkflowTemplate.nom)).all()
    ...
    custom_roles = session.exec(select(CustomRole).where(CustomRole.actif).order_by(CustomRole.libelle)).all()
...
def workflow_help(request: Request, current_user=Depends(get_current_user)):
    """
    ...
    return templates.TemplateResponse("pages/workflow_aide.html", get_template_context(request))
...
def list_workflow_templates(session: Session = Depends(get_session), current_user=Depends(get_current_user)):
    """Liste tous les templates de workflow"""
    if current_user.type_user != "admin":
        raise HTTPException(403, "Accès réservé aux administrateurs")

    templates = session.exec(select(WorkflowTemplate).order_by(WorkflowTemplate.nom)).all()
...
def get_template_steps(
    template_id: int, session: Session = Depends(get_session), current_user=Depends(get_current_user)
):
...
async def add_template_step(
    template_id: int, request: Request, session: Session = Depends(get_session), current_user=Depends(get_current_user)
):
...
async def update_workflow_template(
    ...
    try:
        ...
        raise HTTPException(500, f"Erreur lors de la mise à jour : {e!s}")
...
def delete_workflow_template(
    ...
    try:
        ...
        raise HTTPException(500, f"Erreur lors de la désactivation : {e!s}")
...
def list_request_types(
    categorie: str | None = None, session: Session = Depends(get_session), current_user=Depends(get_current_user)
):
...
def list_custom_roles(session: Session = Depends(get_session), current_user=Depends(get_current_user)):
    """Liste tous les rôles personnalisés actifs"""
    roles = session.exec(select(CustomRole).where(CustomRole.actif)).all()
...
async def update_custom_role(
    role_id: int, request: Request, session: Session = Depends(get_session), current_user=Depends(get_current_user)
):
...
def get_role_assignments(role_id: int, session: Session = Depends(get_session), current_user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(500, f"Erreur lors de la récupération : {e!s}")
...
async def assign_custom_role(
    ...
    try:
        ...
        raise HTTPException(500, f"Erreur lors de l'attribution : {e!s}")


@router.delete("/api/custom-roles/assignments/{assignment_id}", name="api_revoke_role_assignment")
async def revoke_role_assignment(
    assignment_id: int, session: Session = Depends(get_session), current_user=Depends(get_current_user)
):
    ...
    try:
        ...
        raise HTTPException(500, f"Erreur lors de la révocation : {e!s}")
...
def initialize_system_workflows(session: Session = Depends(get_session), current_user=Depends(get_current_user)):
    ...
    try:
        ...
        raise HTTPException(500, f"Erreur lors de l'initialisation : {e!s}")
