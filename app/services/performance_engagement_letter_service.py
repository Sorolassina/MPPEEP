...
class PerformanceEngagementLetterGenerator:
    ...
    def generate_pdf(cls, data: dict[str, Any]) -> BytesIO:
        ...
        logger.info("📄 Page 7+: CHAPITRE III et Signatures")
        cls._draw_chapter_three_and_signatures(pdf, width, height)
    ...
    def _render_multipage_story(
        ...
        *,
        ...
        - Consomme 'story' en place.
        - Numérote les pages.
        - Coupe proprement sans boucle infinie :
          si aucun élément n'est consommé, on sort de la boucle.
        """
        logger = cls.logger
        first_page = True

        while story:
            # La première page est déjà créée par generate_pdf (pdf.showPage() AVANT l'appel)
            if not first_page:
                pdf.showPage()
            ...
            before = len(story)
            logger.info(f"   📝 Page {page_num}: {before} éléments restants")
            frame.addFromList(story, pdf)
            after = len(story)
            consumed = before - after
            logger.info(f"   ✅ Page {page_num}: {consumed} éléments consommés, {after} restants")

            if show_page_number:
                pdf.setFont("Helvetica-Bold", 12)
                pdf.drawRightString(page_width - 30, 25, str(page_num))
            ...
            if after == before and before > 0:
                logger.warning(
                    f"   ⚠️ Aucun Flowable consommé sur la page {page_num}. "
                    f"Sortie de boucle pour éviter une boucle infinie."
                )
                break
    ...
    def _draw_preamble_page(cls, pdf: canvas.Canvas, width: float, height: float) -> None:
        """Pages de préambule (page 3+), avec justification, puces et pagination auto."""
        left_margin   = 2 * cm
        right_margin  = 2 * cm
        top_margin    = 2.5 * cm
        bottom_margin = 2 * cm
        ...
        paragraphs = [
            ...
            "Le programme « Portefeuille de l'État » est institué pour améliorer la performance des entreprises publiques conformément aux objectifs stratégiques, économiques et sociaux de l'État.",
        ]
        for p in paragraphs:
    ...
    def _draw_parties_page(cls, pdf: canvas.Canvas, width: float, height: float) -> None:
        """Section LES PARTIES (page 4+), avec justification et pagination auto."""
        left_margin   = 2 * cm
        right_margin  = 2 * cm
        top_margin    = 2.5 * cm
        bottom_margin = 2 * cm
    ...
    def _draw_chapter_one_page(cls, pdf: canvas.Canvas, width: float, height: float) -> None:
        ...
        - Titre de chapitre au début de la page (generate_pdf doit faire showPage avant).
        - Articles : titres avec keepWithNext pour éviter les titres orphelins.
        - Texte justifié, puces, pagination auto.
        """
        left_margin   = 2 * cm
        right_margin  = 2 * cm
        top_margin    = 2.5 * cm
        ...
        article_title_style = ParagraphStyle(
            ...
            spaceAfter=4,
            spaceBefore=6,
            keepWithNext=1,  # 🔑 évite les titres orphelins
        )
        body_style = ParagraphStyle(
            "ChapterBody",
            parent=styles["Normal"],
    ...
    @classmethod
    def _draw_chapter_three_and_signatures(cls, pdf: canvas.Canvas, width: float, height: float) -> None:
        """
        Dessine le CHAPITRE III (articles 11-14) et les signatures.
        Si le chapitre se termine avec de l'espace, les signatures sont dessinées sur la même page.
        Sinon, une nouvelle page est créée pour les signatures.
        """
        from reportlab.lib.styles import getSampleStyleSheet, ParagraphStyle
        ...
        # Créer un Flowable personnalisé pour les signatures
        class SignaturesFlowable(Flowable):
            def __init__(self, data, available_width):
                Flowable.__init__(self)
                self.data = data
                self.available_width = available_width
                # Hauteur approximative nécessaire pour les signatures
                self.height = 8 * cm
                self.width = available_width
            
            def draw(self):
                # Les coordonnées sont relatives au Flowable (0,0 en bas à gauche)
                # On dessine les signatures en partant du haut du Flowable
                flowable_height = self.height
                
                # Section de signatures - "Fait à ..."
                ville = self.data.get("ville_signature", "Abidjan")
                self.canv.setFont("Helvetica", 12)
                self.canv.drawRightString(self.available_width, flowable_height - 1 * cm, f"Fait à {ville}, le…………………………")

                # Zone de signature Responsable du Programme (gauche)
                programme = self.data.get("programme_intitule", "PORTEFEUILLE DE L'ETAT")
                rprog_nom = self.data.get("rprog_nom", "Nom Prénom").upper()
                self.canv.setFont("Helvetica", 11)
                y_pos = flowable_height - 3.5 * cm
                self.canv.drawCentredString(self.available_width * 0.25, y_pos, "Le Responsable du Programme")
                y_pos -= 16
                self.canv.drawCentredString(self.available_width * 0.25, y_pos, f"« {programme} »")
                y_pos -= 64
                self.canv.setFont("Helvetica-Bold", 11)
                self.canv.drawCentredString(self.available_width * 0.25, y_pos, rprog_nom)

                # Zone de signature Ministre (droite)
                minister_civility = self.data.get("minister_civility", "Monsieur")
                minister_nom = self.data.get("minister_nom", "MOUSSA SANOGO").upper()
                minister_fonction = self.data.get("minister_fonction", "MINISTRE DU PATRIMOINE, DU PORTEFEUILLE DE L'ETAT ET DES ENTREPRISES PUBLIQUES")
                ...
                self.canv.setFont("Helvetica", 11)
                y_pos = flowable_height - 3.5 * cm
                self.canv.drawCentredString(self.available_width * 0.75, y_pos, "Le Ministre du Patrimoine")
                y_pos -= 16
                if len(fonction_lines) > 1:
                    self.canv.drawCentredString(self.available_width * 0.75, y_pos, "du Portefeuille de l'Etat")
                    y_pos -= 16
                    self.canv.drawCentredString(self.available_width * 0.75, y_pos, "et des Entreprises Publiques")
                else:
                    self.canv.drawCentredString(self.available_width * 0.75, y_pos, minister_fonction)
                
                y_pos -= 30
                self.canv.setFont("Helvetica-Bold", 11)
                self.canv.drawCentredString(self.available_width * 0.75, y_pos, minister_nom)
        ...
        story.append(Paragraph("Article 13 : Résiliation de la lettre d'engagement", article_title_style))
        ...
        story.append(Paragraph("Article 14 : Date d'effet", article_title_style))
        story.append(Paragraph(article14_text, body_style))
        story.append(Spacer(1, 1.5 * cm))
        
        # Ajouter les signatures comme un élément de la story, juste après l'article 14
        signatures_flowable = SignaturesFlowable(cls.data, available_width)
        story.append(signatures_flowable)

        # Utiliser _render_multipage_story pour gérer la pagination
        page_num = 7
        cls._render_multipage_story(
            pdf,
            story,
