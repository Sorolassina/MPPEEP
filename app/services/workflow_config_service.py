...
class WorkflowConfigService:
    ...
    def create_template(
        ...
        WorkflowConfigService._log_change(
            session, "WorkflowTemplate", template.id, "CREATE", user_id
        )
    ...
    def add_step_to_template(
        ...
        # Enregistrer dans l'historique
        WorkflowConfigService._log_change(
            session, "WorkflowTemplateStep", step.id, "CREATE", user_id,
    ...
    @staticmethod
    def delete_template(session: Session, template_id: int, user_id: Optional[int] = None):
        ...
        if types_utilisant:
            raise ValueError(
                f"Ce template est utilisé par {len(types_utilisant)} type(s) de demande actif(s)"
            )
        ...
        WorkflowConfigService._log_change(
            session, "WorkflowTemplate", template_id, "DEACTIVATE", user_id
        )
    
    @staticmethod
    def delete_template_permanently(session: Session, template_id: int, user_id: Optional[int] = None):
        ...
        WorkflowConfigService._log_change(
            session, "WorkflowTemplate", template_id, "DELETE", user_id
        )
    ...
    def create_request_type(
        ...
        description: Optional[str] = None,
        categorie: str = "Autre",
        champs_formulaire: Optional[List[Dict]] = None,
        ...
        existing = session.exec(
            select(RequestTypeCustom).where(RequestTypeCustom.code == code)
        ).first()
        ...
        WorkflowConfigService._log_change(
            session, "RequestTypeCustom", request_type.id, "CREATE", user_id
        )
        
        return request_type
    
    @staticmethod
    def get_active_request_types(session: Session, categorie: Optional[str] = None) -> List[RequestTypeCustom]:
        """Récupère tous les types de demandes actifs, optionnellement filtrés par catégorie"""
        query = select(RequestTypeCustom).where(RequestTypeCustom.actif == True)
    ...
    @staticmethod
    def get_request_type_by_code(session: Session, code: str) -> Optional[RequestTypeCustom]:
    ...
    def create_custom_role(
        ...
        WorkflowConfigService._log_change(
            session, "CustomRole", role.id, "CREATE", user_id
        )
    ...
    def assign_role_to_agent(
        ...
        if existing:
            raise ValueError(f"L'agent a déjà ce rôle (attribution active)")
        ...
        WorkflowConfigService._log_change(
            session, "CustomRoleAssignment", assignment.id, "CREATE", user_id,
    ...
    def get_agents_with_role(session: Session, custom_role_id: int) -> List[AgentComplet]:
        ...
        return session.exec(
            select(AgentComplet)
            .where(AgentComplet.id.in_(agent_ids))
    ...
    def initialize_system_workflows(session: Session):
        ...
        template_long = session.exec(
            select(WorkflowTemplate).where(WorkflowTemplate.code == "CIRCUIT_LONG_RH")
        ).first()
        ...
        template_moyen = session.exec(
            select(WorkflowTemplate).where(WorkflowTemplate.code == "CIRCUIT_MOYEN")
        ).first()
    ...
    def get_workflow_preview(session: Session, template_id: int) -> Dict:
        ...
        if template.direction == WorkflowDirection.ASCENDANT:
            preview['circuit'].append({
                'etape': 0,
                'role': 'DEMANDEUR',
                'libelle': 'Demandeur (Agent)',
                'action': 'Soumet la demande'
            })
