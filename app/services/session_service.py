...
class SessionService:
    ...
    @staticmethod
    def create_session(db_session: Session, user: User, request: Request, remember_me: bool = False) -> UserSession:
        """
        ...
        logger.info(
            f"✅ Session créée pour l'utilisateur {user.email} (ID: {user.id}, Device: {device_info}, IP: {client_ip})"
        )

        return user_session

    @staticmethod
    def get_session_by_token(db_session: Session, session_token: str) -> UserSession | None:
        """
        ...
        """
        statement = select(UserSession).where(UserSession.session_token == session_token, UserSession.is_active)
    ...
    @staticmethod
    def get_user_from_session(db_session: Session, session_token: str) -> User | None:
        """
    ...
    @staticmethod
    def delete_session(db_session: Session, session_token: str) -> bool:
        """
    ...
    @staticmethod
    def delete_all_user_sessions(db_session: Session, user_id: int) -> int:
        """
        ...
        """
        statement = select(UserSession).where(UserSession.user_id == user_id, UserSession.is_active)
    ...
    @staticmethod
    def get_active_sessions(db_session: Session, user_id: int) -> list[UserSession]:
        """
        ...
        statement = (
            select(UserSession)
            .where(UserSession.user_id == user_id, UserSession.is_active, UserSession.expires_at > datetime.now())
    ...
    def cleanup_expired_sessions(db_session: Session) -> int:
        ...
        """
        statement = select(UserSession).where(UserSession.is_active, UserSession.expires_at < datetime.now())
        sessions = db_session.exec(statement).all()
