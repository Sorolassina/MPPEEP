...
class StockService:
    ...
    def get_kpis(session: Session) -> dict[str, Any]:
        ...
        # Articles actifs
        articles_actifs = session.exec(select(func.count(Article.id)).where(Article.actif)).one()

        # Articles en rupture (quantité <= quantité_min)
        articles_rupture = session.exec(
            select(func.count(Article.id)).where(Article.quantite_stock <= Article.quantite_min)
        ).one()
        ...
        demandes_en_attente = session.exec(
            select(func.count(DemandeStock.id)).where(DemandeStock.statut == "EN_ATTENTE")
        ).one()

        demandes_validees = session.exec(
            select(func.count(DemandeStock.id)).where(DemandeStock.statut == "VALIDEE")
        ).one()
        ...
        mouvements_entree = session.exec(
            select(func.count(MouvementStock.id)).where(
                MouvementStock.type_mouvement == "ENTREE", MouvementStock.date_mouvement >= premier_jour_mois
            )
        ).one()

        mouvements_sortie = session.exec(
            select(func.count(MouvementStock.id)).where(
                MouvementStock.type_mouvement == "SORTIE", MouvementStock.date_mouvement >= premier_jour_mois
            )
        ).one()
    ...
    def creer_article(
        ...
        **kwargs,
        ...
        existing = session.exec(select(Article).where(Article.code == code)).first()
        ...
        **kwargs,
        ...
        if quantite_apres <= article.quantite_min:
            if quantite_apres == 0:
                ...
                logger.warning(
                    f"Stock faible : Article {article.code} - Stock: {quantite_apres}, Min: {article.quantite_min}"
                )
    ...
    def creer_demande(
        ...
        **kwargs,
        ...
        count = session.exec(
            select(func.count(DemandeStock.id)).where(DemandeStock.numero.like(f"DEM-{annee}-%"))
        ).one()
    ...
    @staticmethod
    def valider_demande(
        session: Session, demande_id: int, valideur_id: int, accepte: bool, commentaire: str | None = None
    ) -> tuple[DemandeStock, str | None]:
        """
        ...
        if accepte and demande.type_demande == "SORTIE":
            disponible, msg = StockService.verifier_stock_disponible(
                session=session, article_id=demande.article_id, quantite_demandee=demande.quantite_demandee
            )
            if not disponible:
    ...
    @staticmethod
    def creer_inventaire(session: Session, libelle: str, responsable_id: int, date_debut: date) -> Inventaire:
        """Crée un nouvel inventaire"""

        # Générer le numéro
        annee = date_debut.year
        count = session.exec(select(func.count(Inventaire.id)).where(Inventaire.numero.like(f"INV-{annee}-%"))).one()
        numero = f"INV-{annee}-{str(count + 1).zfill(3)}"

        inventaire = Inventaire(numero=numero, libelle=libelle, responsable_id=responsable_id, date_debut=date_debut)
        ...
        for article in articles:
            ligne = LigneInventaire(
                inventaire_id=inventaire.id, article_id=article.id, quantite_theorique=article.quantite_stock
            )
            session.add(ligne)
    ...
    def get_articles_rupture(session: Session) -> list[Article]:
        ...
        articles = session.exec(
            select(Article)
            .where(Article.actif, Article.quantite_stock <= Article.quantite_min)
    ...
    def verifier_stock_disponible(
        session: Session, article_id: int, quantite_demandee: Decimal
    ) -> tuple[bool, str | None]:
        """
        ...
        if stock_apres < 0:
            return (
                False,
                f"Stock insuffisant : Disponible {stock_actuel} {article.unite}, Demandé {quantite_demandee} {article.unite}",
            )
    ...
    def get_alertes_stock(session: Session) -> dict[str, Any]:
        ...
        articles_surstock = session.exec(
            select(Article).where(
                Article.actif, Article.quantite_max > 0, Article.quantite_stock >= Article.quantite_max * Decimal("0.9")
            )
        ).all()
    ...
    def creer_lot_perissable(
        ...
        existing = session.exec(
            select(LotPerissable).where(LotPerissable.article_id == article_id, LotPerissable.numero_lot == numero_lot)
        ).first()
        ...
        logger.info(
            f"Lot périssable créé : {numero_lot} - Article: {article.designation} - Péremption: {date_peremption}"
        )
        return lot
    ...
    def get_lots_perissables_article(session: Session, article_id: int) -> list[LotPerissable]:
        ...
        lots = session.exec(
            select(LotPerissable).where(LotPerissable.article_id == article_id).order_by(LotPerissable.date_peremption)
        ).all()
    ...
    def get_lots_perimes(session: Session) -> list[dict[str, Any]]:
        ...
        lots = session.exec(
            select(LotPerissable)
            .where(LotPerissable.date_peremption < date.today(), LotPerissable.quantite_restante > 0)
            .order_by(LotPerissable.date_peremption)
        ).all()
        ...
        for lot in lots:
            ...
            resultat.append({"lot": lot, "article": article, "jours_perime": jours_perime})
    ...
    def mettre_a_jour_statuts_lots(session: Session) -> dict[str, int]:
        ...
        stats = {"actifs": 0, "alertes": 0, "perimes": 0, "epuises": 0}
        ...
        logger.info(
            f"Statuts lots mis à jour - Actifs: {stats['actifs']}, Alertes: {stats['alertes']}, Périmés: {stats['perimes']}, Épuisés: {stats['epuises']}"
        )
        return stats

    @staticmethod
    def consommer_lot_fifo(session: Session, article_id: int, quantite: Decimal) -> list[dict[str, Any]]:
        """
        ...
        if quantite_restante > 0:
            raise ValueError(
                f"Stock insuffisant : {quantite_restante} {session.get(Article, article_id).unite} manquant(s)"
            )
    ...
    def calculer_amortissement_lineaire(
        valeur_acquisition: Decimal, duree_annees: int, valeur_residuelle: Decimal = Decimal(0)
    ) -> Decimal:
        """
    ...
    @staticmethod
    def calculer_amortissement_degressif(valeur_nette_debut: Decimal, taux_degressif: Decimal) -> Decimal:
        """
    ...
    @staticmethod
    def calculer_amortissement_annee(session: Session, article_id: int, annee: int, user_id: int) -> Amortissement:
        """
        ...
        existing = session.exec(
            select(Amortissement).where(Amortissement.article_id == article_id, Amortissement.annee == annee)
        ).first()
        ...
        amort_precedent = session.exec(
            select(Amortissement).where(Amortissement.article_id == article_id, Amortissement.annee == annee - 1)
        ).first()
        ...
        if amort_precedent:
            ...
            if annee > annee_acquisition:
                logger.warning("Amortissement(s) manquant(s) pour les années précédentes")
        ...
        if article.methode_amortissement == "LINEAIRE":
            amortissement_annuel = StockService.calculer_amortissement_lineaire(
                article.valeur_acquisition, article.duree_amortissement_annees, valeur_residuelle
            )
            ...
            amortissement_annuel = StockService.calculer_amortissement_degressif(
                valeur_nette_debut, article.taux_amortissement
            )
        ...
        logger.info(
            f"Amortissement calculé pour {article.designation} - Année {annee} - Montant: {amortissement_annuel}"
        )
    ...
    def get_plan_amortissement(session: Session, article_id: int) -> list[dict[str, Any]]:
        ...
        amortissements_calcules = session.exec(
            select(Amortissement).where(Amortissement.article_id == article_id).order_by(Amortissement.annee)
        ).all()
        ...
        for i in range(int(derniere_annee_calculee - annee_debut + 1), article.duree_amortissement_annees):
            ...
            if article.methode_amortissement == "LINEAIRE":
                ...
                amort_annuel = StockService.calculer_amortissement_degressif(
                    valeur_nette, article.taux_amortissement or Decimal(0)
                )
    ...
    @staticmethod
    def get_materiels_a_amortir(session: Session, annee: int | None = None) -> list[dict[str, Any]]:
        """
        ...
        materiels = session.exec(
            select(Article).where(Article.est_amortissable, Article.actif, Article.date_acquisition.is_not(None))
        ).all()
        ...
        for materiel in materiels:
            ...
            amort_existe = session.exec(
                select(Amortissement).where(Amortissement.article_id == materiel.id, Amortissement.annee == annee)
            ).first()
