...
from app.models.budget import (
    FicheTechnique, ActionBudgetaire, ServiceBeneficiaire, 
    ActiviteBudgetaire, LigneBudgetaireDetail
)
...
class FicheTechniqueService:
    ...
    def analyser_fichier_excel(
        ...
        try:
            ...
            result = FicheTechniqueService._creer_structure_hierarchique(
                df, colonnes_mappees, fiche.id, session
            )
    ...
    def _valider_et_mapper_colonnes(df: pd.DataFrame) -> Dict[str, str]:
        ...
        """
        logger.info(f"🔍 Validation du template fiche technique...")
    ...
    def _creer_structure_hierarchique(
        ...
        for idx, row in df.iterrows():
            try:
                ...
                # Nature de dépenses
                if code_libelle.upper() in ["BIENS ET SERVICES", "PERSONNEL", "INVESTISSEMENT", "INVESTISSEMENTS", "TRANSFERTS"]:
                ...
                # Service
                if code_libelle.strip().startswith("Service Bénéficiaire :") or code_libelle.strip().startswith("- Service Bénéficiaire :"):
                    if current_action:
        ...
        logger.info(f"📊 Résumé : {actions_count} actions, {services_count} services, {activites_count} activités, {lignes_count} lignes")
    ...
    def _creer_action(
        ...
        action = ActionBudgetaire(
            ...
            justificatifs=str(row[colonnes_mappees["justificatifs"]]) if "justificatifs" in colonnes_mappees and pd.notna(row[colonnes_mappees["justificatifs"]]) else None
        )
    ...
    def _recalculer_totaux_hierarchie(fiche_id: int, session: Session):
        ...
        activites = session.exec(
            ...
            .where(ActiviteBudgetaire.fiche_technique_id == fiche_id)
        ).all()
