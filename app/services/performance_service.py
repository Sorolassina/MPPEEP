...
from app.models.performance import (
    ObjectifPerformance, IndicateurPerformance, ProgrammePerformance, EvaluationPerformance,
    StatutObjectif, PrioriteObjectif, TypeObjectif
)
...
class PerformanceService:
    ...
    @staticmethod
    def get_objectif(session: Session, objectif_id: int) -> Optional[ObjectifPerformance]:
    ...
    @staticmethod
    def get_objectifs(session: Session, skip: int = 0, limit: int = 100, 
                     statut: Optional[str] = None, 
                     responsable_id: Optional[int] = None,
                     type_objectif: Optional[str] = None) -> List[ObjectifPerformance]:
    ...
    @staticmethod
    def modifier_objectif(session: Session, objectif_id: int, objectif_data: Dict[str, Any]) -> Optional[ObjectifPerformance]:
        ...
        try:
            ...
            if objectif.date_fin < date.today() and objectif.statut not in [StatutObjectif.ATTEINT, StatutObjectif.ANNULE]:
    ...
    def get_kpis_objectifs(session: Session) -> Dict[str, Any]:
        ...
        try:
            ...
            # Progression moyenne
            progression_moyenne = session.exec(
    ...
    @staticmethod
    def creer_indicateur(session: Session, indicateur_data: Dict[str, Any], created_by_id: int) -> IndicateurPerformance:
    ...
    @staticmethod
    def get_indicateurs(session: Session, skip: int = 0, limit: int = 100, 
                       categorie: Optional[str] = None,
                       responsable_id: Optional[int] = None,
                       frequence_mesure: Optional[str] = None,
                       actif_only: bool = True) -> List[IndicateurPerformance]:
        ...
        try:
            ...
            if actif_only:
                conditions.append(IndicateurPerformance.actif == True)
    ...
    @staticmethod
    def get_indicateur(session: Session, indicateur_id: int) -> Optional[IndicateurPerformance]:
    ...
    @staticmethod
    def modifier_indicateur(session: Session, indicateur_id: int, indicateur_data: Dict[str, Any]) -> Optional[IndicateurPerformance]:
        """Modifie un indicateur existant"""
        try:
            indicateur = session.exec(select(IndicateurPerformance).where(IndicateurPerformance.id == indicateur_id)).first()
            if not indicateur:
                return None
